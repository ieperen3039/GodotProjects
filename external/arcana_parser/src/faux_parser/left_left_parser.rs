...
impl<'c> Parser {
    ...
    pub fn parse_program<'prog: 'c>(
        ...
        for failure in failures {
            let minimum = parser::get_err_significance(furthest_failures.last().unwrap())
                - parser::MAX_NUM_TOKENS_BACKTRACE_ON_ERROR;
            if parser::get_err_significance(&failure) > minimum {
                furthest_failures.push(failure);
                furthest_failures.sort_by(parser::compare_err_result);
            }
        }
